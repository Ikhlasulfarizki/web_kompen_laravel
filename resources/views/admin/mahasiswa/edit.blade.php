    ...
    <div class="bg-white rounded-lg shadow-lg p-8 max-w-2xl mx-auto">
        <form action="{{ route('admin.mahasiswa.update', $mahasiswa->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="npm" name="npm" value="{{ old('npm', $mahasiswa->npm) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('npm') border-red-500 @enderror">
                @error('npm')
            ...
            <div class="mb-6">
                ...
                <input type="text" id="nama" name="nama" value="{{ old('nama', $mahasiswa->nama) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('nama') border-red-500 @enderror">
                @error('nama')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="date" id="tgl_lahir" name="tgl_lahir" value="{{ old('tgl_lahir', $mahasiswa->tgl_lahir) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('tgl_lahir') border-red-500 @enderror">
                @error('tgl_lahir')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="id_jurusan" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">
                    ...
                    @foreach($jurusan as $j)
                        <option value="{{ $j->id }}" {{ $mahasiswa->kelas->prodi->jurusan->id == $j->id ? 'selected' : '' }}>{{ $j->nama_jurusan }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="id_prodi" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">
                    <option value="{{ $mahasiswa->kelas->prodi->id }}" selected>{{ $mahasiswa->kelas->prodi->nama_prodi }}</option>
                </select>
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="id_kelas" name="id_kelas" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('id_kelas') border-red-500 @enderror">
                    <option value="{{ $mahasiswa->id_kelas }}" selected>{{ $mahasiswa->kelas->nama_kelas }}</option>
                </select>
                @error('id_kelas')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
